...
| `principle-drift` | `/minder:mem:process` | `/minder:mem:check-decision` verdict violated at confidence ≥ 0.8 on an eligible artefact from the current batch — a knowledge note typed `decision`, or a record with `kind: observation` that the subagent flagged into `tradeoff_framing_records[]`, per §3.7.5. Each path selects on a field the layer it targets actually carries | Capture in trail; behaviour unchanged this batch |
...
| `extraction-source-missing` | `/minder:mem:resolve-clarifications` Step 6.5 | An approved `knowledge-note` has no source to go to: `resolve-clarifications` has no row in `## Active Sources` or `## Reserved Sources` of `SOURCES.md` | The approved text is kept verbatim in the item; nothing else is written |
...
| `scheduler-tick-reverted-content` | `/minder:mem:lint` Scan A.15 | A `[scheduled]` commit that delivered a stale tree: its tree equals an ancestor's on every path it did not itself write, and it carries its own work on top — the shape a tick produces when it folds onto an `origin/main` that moved while it ran, reverting whatever arrived in between. Provenance plus that shape plus own-work-on-top separates it from a deliberate revert, which reinstates and adds nothing; a fourth condition — somebody else's work inside the reverted interval — separates it from a scheduled run rolling back its own previous output, which satisfies the first three. A commit meeting the first three but not the fourth is surfaced in the same item as needing the owner's verdict, never dropped: a same-tag race and a deliberate self-rollback are indistinguishable in the history. A commit that declares its base (`Minder-Tick-Base`, written by `finalize-tick.sh`) is judged by that base alone: a path put back to its base content while the parent holds something newer reverted work the tick never read, and a base equal to the parent never did — so a declared commit is proven or clean, never left for review | Surface as one aggregate naming the commits and the count of reverted paths; never autofixed. The repair is `scripts/recover_reverted_ticks.py apply`, which writes into the working tree for the owner to review — restoring content and reconciling the state files that describe it are one decision, and lint's write territory covers neither |
| `scheduler-tick-identity-missing` | `/minder:mem:lint` Scan A.15 | A `[scheduled]` commit newer than the first commit in the history that declares its base, which itself declares nothing usable: no `Minder-Tick-Base`, a malformed one, or a base that does not exist or does not precede the commit. The ordinary cause is a delivery route that dropped the message body — a routine still holding a pasted prompt body whose Step 5b predates the declaration, or a machine running an older engine. Such a commit is judged from history alone, so a stale delivery by it could land back in `needs_review` | Surface as one aggregate naming the commits and each one's reason; never autofixed. The fix is upstream of the commit — replace a pasted routine body with the loader, update the engine on the machine that delivered it — because a commit's message cannot be amended once on `main` |
...
- `.scheduler-state/staged-paths` (gitignored, outside the base) — written by **every step of a tick that stages or commits**: `scripts/scheduler/stage.sh` on each run, and any skill making its own commit mid-tick, which today is `/minder:mem:roles` alone, once per role. Append-only within a tick, removed only when a delivery succeeds. `finalize-tick.sh` reads it as the set of paths the tick is allowed to deliver and refuses to push anything outside it, so the rule is general rather than one skill's habit: **a step records what it staged, at the moment it stages it, before committing** — afterwards the index is clean and there is nothing left to read. Two directions that are easy to get backwards: the record is never derived from the commits it polices (after the fold a folded commit's changes are back in the index, and a surface read from there would authorise the stale tree the check exists to catch), and a missing or empty record fails **closed** — every differing path becomes unauthorised, so a skill that forgets to record finds its own work refused rather than quietly published. Full rationale: `docs/scheduling.md` → «The delivery surface»
...
|---|---|---|
| Create new records / notes / tasks / events | `/minder:mem:process` only; a person profile also `/minder:mem:resolve-clarifications` on the owner's approval | Extraction from sources is the process domain; knowledge from a resolve session reaches it through the inbox |
...
| Modify body of existing records/notes | `/minder:mem:process` (initial) + `/minder:mem:lint` (dedup merge only) + `/minder:mem:resolve-clarifications` (owner-approved diffs) | Any other content write is a row below |
...
| Drop a file in `_sources/inbox/resolve-clarifications/` | `/minder:mem:resolve-clarifications` Step 6.5, on the owner's approval | The only path from a resolve session into knowledge. Flat file, shape in that step; `/minder:mem:process` then treats it as any other source |
...
там. Файл или раздел не читается — `/minder:mem:process` завершает прогон
целиком, включая metric-day, ошибкой, называющей этот файл: без правил заметка
знаний не пишется.
