...
| resolve-clarifications | `_sources/inbox/resolve-clarifications/` | transcript | flat-md | auto | — | Knowledge the owner approved in a `/minder:mem:resolve-clarifications` session — one flat file per approved answer, holding the question and the owner's words; `/minder:mem:process` folds it in like any source. Producer and file shape: that skill's Step 6.5. | active |
