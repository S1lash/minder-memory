# Minder Memory — Журнал решений (ADR)
...
### ADR-029: Продукт называется Minder Memory; namespace скиллов `minder:mem:*`

**Контекст:** Minder становится экосистемой продуктов (Minder Memory, Minder
Harness, Minder Listen …). Вторая память нуждалась в собственном имени внутри
неё; «Minder Memory» было именем метода, из которого продукт вырос (Zettelkasten), а не
именем продукта. Переименование затрагивало движок, данные владельца, публичный
скелет, установщик, соседние репозитории и харнесы друзей — доставка обязана
была пройти через обычный `update` с миграцией.

**Решение:**
- Продукт — **Minder Memory**, короткая форма **Minder Mem**, слаг
  `minder-memory`. Namespace скиллов — `/minder:mem:<name>` (двухуровневый:
  экосистема полным словом, продукт — санкционированным коротким алиасом;
  оставляет `/minder:harness:*`, `/minder:listen:*` свободными). Директории
  скиллов `minder-mem-<name>` (NTFS не допускает `:`), frontmatter
  `name: minder:mem:<name>`; команды переезжают в `commands/minder/mem/`.
- Папка данных `zettelkasten/` остаётся: это имя метода, а не продукта, и
  корень всех путей у друзей.
- Прежнее имя остаётся **навсегда как разговорный алиас** («найди в Minder Memory»,
  «Minder Memory»), в том числе STT-искажённые формы нового имени — в горячем правиле.
- Переменные среды `MINDER_MEMORY_BASE` / `MINDER_MEMORY_ROLES_KEY` — без
  fallback на прежние имена: две переменные для одного факта — дефект, а не
  совместимость. Переименование в окружении планировщика делает агент по
  чек-листу апгрейда (`docs/upgrade-1.0.0.md`, шаг 7.2 `/minder:mem:update`).
- Контракт манифеста: `processor: minder:mem:<pipeline>`, одно написание;
  батчи и буферы на диске переписываются миграцией (append-only файлы
  сохраняют порядок и каждую строку — меняется только имя).
- Доставка: миграция 031 (`structural`) перевязывает `~/.claude` и
  переписывает `upstream` только после того, как новый адрес ответил;
  миграция 032 (`heal`) применяет карту переименования ко всему клону —
  единственная перезапись owner-данных, которую движок делает без запроса
  (§3.6 доктрины): детерминированная, идемпотентная, обратимая через git;
  `_sources/` не трогает. Переименование дашборда — цепочка в `seed.sh`.
- Цепочка миграций начинается с 031; исполненные 002–030 удалены из движка.
  Клон старше 0.69.0 обновляется до 0.69.0 (`sync_engine.sh --self-heal
  --branch release/0.69.0` — ветка в скелете; self-heal сначала ставит
  машинерию обновления того релиза), затем до текущего; прямой прыжок
  `sync_engine.sh` отказывает.
- Стиль имени — одно правило (`CONVENTIONS.md → «Имя продукта»`): `mem`
  только в пространстве команд и производных от него; везде иначе — полное
  `memory`.
- Версия движка 1.0.0.

**Альтернативы:**
- `/memory:*`, `/mm:*`, `/mnd:mem:*` — отвергнуты: первые два без
  экосистемного префикса, третий — аббревиатура имени, которое продукт нигде
  больше не сокращает.
- Alias-скиллы `/minder:mem:*` на один релиз — отвергнуты владельцем: чистый разрыв,
  разговорный алиас закрывает recall.
- Переименовать `zettelkasten/` в `memory/` — отвергнуто: второй ребрендинг с
  другим радиусом (вольты, MCP-конфиги, рутины друзей).
- Оставить fallback на прежние имена переменных и старое написание
  `processor` валидным «на всякий случай» — отвергнуто: рудимент, который
  читают два места, — это два имени одного факта; переименование в окружении
  друга делает агент, а не тихий второй код-путь.
- Спрашивать владельца перед переименованием его файлов (CLARIFICATION с
  opt-in) — отвергнуто: имя продукта — факт о продукте, а не о владельце;
  половина базы со старым именем — тот дрейф, от которого §3.6 защищает.

**Следствие:** одна карта переименования (`_032_minder_memory_rebrand.py`)
— единственный дом токенов, защищённых span'ов и code-safe подмножества;
`sync_engine.sh` читает список engine-путей из манифеста, который собирается
применить (латентный дефект, вскрытый переименованием: путь, существующий
только в новом манифесте, не доезжал на том же обновлении); идентификаторы в
коде соседних репозиториев (ключи конфигов коллекторов, имена модулей плагина,
живые хостнеймы) остаются под прежними именами до отдельного этапа.
Дизайн и закрытие цикла — `platform/minder-memory-rebrand/`.

### ADR-030: Миграция вправе ремонтировать данные владельца — узким классом

**Контекст:** `finalize-tick.sh` складывал коммиты тика через
`git reset --soft origin/main` после `fetch`. Если за время тика кончик уехал,
индекс со старым деревом коммитился поверх нового и откатывал всё, что пришло
между стартом и финишем; push при этом — обычный fast-forward, отказывать ему
git не за что. Шесть доставок с 04.09 по 11.09 стёрли по дню интейка каждая, а
поскольку в том же коммите откатывались и state-файлы, описывающие работу, ни
один контентный скан этого не видит: пропадает событие, а не файл. У друзей
рутины продолжают работать, то есть их базы ломаются прямо сейчас.

Правило `scripts/migrations/README.md` при этом запрещало миграциям трогать
данные владельца, а единственный прецедент обратного — ADR-029 (переименование
продукта, миграция `032`).

**Решение:** миграция вправе писать в данные владельца, когда выполнены **все
пять** условий: (1) состояние, которое она правит, вызвано дефектом самого
движка; (2) содержимое берётся из истории этого же клона, ничего не
изобретается; (3) операция детерминирована и идемпотентна; (4) всё, что не
решается правилом, не угадывается, а передаётся агенту обновления или
владельцу; (5) `git` отменяет её одним шагом — значит она никогда не пишет в
файл с незакоммиченной работой владельца. Восстановление автоматическое, по
прямому решению владельца: ждать, пока друг прочитает clarification, значит
оставить его базу сломанной.

Писать разрешает **не признак, а доказательство**: `[scheduled]`-провенанс,
плюс дерево, совпадающее с предком везде, где коммит ничего не писал, плюс
собственная работа сверху, которой у намеренного реверта не бывает, плюс
**чужая работа внутри откачённого интервала**. Четвёртое условие добавлено по
находке ревью и отделяет дефект от планового прогона, который осознанно
откатывает свой же предыдущий результат: такой прогон удовлетворяет первым трём,
потому что попутно дописывает свою строку в лог. На всей истории (2765 коммитов,
47 совпадений признака) это называет ровно шесть инцидентов и отвергает оба
намеренных реверта с нулевым расхождением, волны ребрендинга, чистки и сам
восстановительный коммит 08.09.

Доказательство честно не всесильно: две гонки тиков **с одним и тем же тегом**
неотличимы в истории от осознанного самоотката. Такой коммит не пишется и не
отбрасывается — он получает вердикт `needs_review` и выносится владельцу, потому
что молчание здесь и есть тот путь, которым реальная потеря отчитывается как
чистая база. Провенанс из темы коммита — не идентичность; для будущих коммитов
это лечится записью run-id в сам коммит (follow-up в DESIGN), задним числом —
нет.

**Альтернативы:**
- Только детект плюс clarification — отвергнуто владельцем: у друга нет
  контекста, который есть у агента, стоящего в его базе с загруженным каноном;
  база остаётся сломанной до ручного разбора.
- Угадывать по ближайшему правилу — отвергнуто: догадка пишет в заметки
  владельца, и её ошибку не видно, потому что вернувшаяся заметка выглядит
  как никогда не терявшаяся.
- Разовый скрипт для своей базы и отдельная миграция для друзей — отвергнуто:
  две реализации разойдутся, и разойдётся именно та, которую никто не
  прогоняет.
- Оставить запрет в README и обойти его молча — отвергнуто: правило, тихо
  нарушенное движком, который его написал, хуже отсутствующего правила.

**Следствие:** `scripts/recover_reverted_ticks.py` — единственный дом детекции,
доказательства и ремонта; его вызывают трое: восстановление своей базы,
миграция `035` и `/minder:mem:lint` A.15 (тип
`scheduler-tick-reverted-content` зарегистрирован в `SYSTEM_CONFIG.md`).
README миграций несёт пять условий и называет `032` и `035` как
квалифицирующиеся. Негативные фикстуры взяты реальными коммитами, а не
придуманными. Опт-аут — `MINDER_MEMORY_NO_AUTO_RECOVER=1`. Дизайн и закрытие
цикла — `platform/scheduler-delivery-recovery/`.
