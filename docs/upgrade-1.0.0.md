...
| 7 | The clone's own files | `git grep -il ztn -- . ':!zettelkasten/_sources' ':!docs/CHANGELOG.md' ':!zettelkasten/5_meta/DECISION_LOG.md' ':!scripts/migrations' ':!scripts/check_update.py' ':!zettelkasten/_system/scripts/tests' ':!integrations/claude-code/rules/minder-memory.md' ':!.engine-manifest.yml' ':!integrations/obsidian/seed.sh' ':!platform' ':!docs/upgrade-1.0.0.md' ':!zettelkasten/5_meta/help/CHANGELOG.md' ':!zettelkasten/5_meta/help/.pre-refresh-backup' ':!zettelkasten/_system/state/CLARIFICATIONS.md' ':!zettelkasten/_system/state/secrets.enc.json'` prints nothing — the excluded places are the alias list, the two history documents (and the vault's derived copy of the changelog), the migration and its tests plus the post-update check `scripts/check_update.py` (whose inputs — and whose subject — are the former form), the manifest's `retired:` rows and the dashboard chain in `seed.sh` (former names by definition), the clarifications queue (where the engine RAISES a former name with you), the pre-refresh backup of the vault help (kept precisely so the former wording survives), the credential store (a key there is YOUR name for a service and is never rewritten), and the maintainer's design records. `_sources/` is excluded because a transcript is evidence of what was said — the one file the sync does rewrite in there is the engine-shipped template under `_sources/inbox/describe-me/`, which is engine, not a recording of yours. The proof is that it prints nothing except lines that name something of YOURS — a repository or folder (`minder-ztn-<something>`) or a credential (`ZTN_<SOMETHING>` that is not one of the engine's own variables) — those are kept on purpose, and `scripts/check_update.py` discounts them together with the rename's two opt-outs: a line ending `rebrand:keep`, and anything inside a file carrying `minder-memory-rebrand: keep-legacy-tokens`. Both say the former spelling is the point of the line |
