...
| 6 | Upstream remote | `git remote get-url upstream` ends in `/minder-memory.git` (or `/minder-memory`) |
| 7 | The clone's own files | `git grep -il ztn -- . ':!zettelkasten/_sources' ':!docs/CHANGELOG.md' ':!zettelkasten/5_meta/DECISION_LOG.md' ':!scripts/migrations' ':!zettelkasten/_system/scripts/tests' ':!integrations/claude-code/rules/minder-memory.md' ':!.engine-manifest.yml' ':!integrations/obsidian/seed.sh' ':!platform' ':!docs/upgrade-1.0.0.md' ':!zettelkasten/5_meta/help/CHANGELOG.md'` prints nothing — the excluded places are the alias list, the two history documents (and the vault's derived copy of the changelog), the migration and its tests (whose inputs are the former form), the manifest's `retired:` rows and the dashboard chain in `seed.sh` (former names by definition), and the maintainer's design records |
...
| 15 | Collectors and other producers into the inbox | a collector's config that names the repository by its former GitHub name or holds a former variable | the assistant lists what it can see (`~/.minder-*/config.json`, launchd plists); the owner applies on any machine or server it cannot see | the next collector run pushes into the inbox under the new repository name |
| 16 | Repository names | the owner's own clone and remotes point at the renamed GitHub repository | `git remote -v` names `minder-memory…`; a stale name still works through the redirect but is not left in place | `git remote -v` shows no former name |
